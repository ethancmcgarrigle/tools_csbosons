...
for i in range(num_tpoints): 
  dynamical_partnum[i] = np.mean(cols[first_indx + 2*i] + 1j*cols[first_indx + 2*i] ) 
  dynamical_partnum_err[i] = stats.sem(cols[first_indx + 2*i] + 1j*cols[first_indx + 2*i] ) 
...
plt.legend()
plt.ylim(N_avg_eqb - 0.1*N_avg_eqb, N_avg_eqb + 0.1*N_avg_eqb)
plt.show()
