...
with open('input.yml') as infile:
 ...
 ##plt.xlabel(r'$\tilde x$', fontsize = 24, fontweight = 'bold')
 ##plt.ylabel(r'$\tilde y$', fontsize = 24, fontweight = 'bold')
 ##plt.colorbar()
 #plt.clim(0, 81)
 #plt.savefig('Fig1c_manyN_86mm_K12_T16.eps', dpi = 300)
...
normalizing = np.abs(np.max(density_up.real))

#print('average density: ' + str(np.round(avg_rho,3)))

plt.figure(figsize=(3.38583, 3.38583))
plt.imshow(density_up.real/normalizing, cmap = sns_cmap, interpolation='none', extent=[np.min(x) ,np.max(x) ,np.min(y),np.max(y)]) 
#plt.colorbar()
cbar = plt.colorbar()
...
np.savetxt('emu_T300nK_trapped_data_wavefxn.dat', density_up.real/normalizing)
