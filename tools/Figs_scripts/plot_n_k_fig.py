...
np.savetxt('n_k_figure_data.dat', n_k_sorted.real)
...
#plt.figure(1)
#plt.figure(figsize=(2.204, 2.204))
#data = np.loadtxt('n_k_figure_data.dat')
...
plt.imshow(n_k_sorted.real, cmap = 'hot', interpolation='none', extent=[np.min(kx), np.max(kx), np.min(ky), np.max(ky)]) 
plt.xlabel('$k_x$', fontsize = 28) 
plt.ylabel('$k_y$', fontsize = 28)
#plt.annotate(r'$\tilde{T} = ' + str(17.9) + '$', (10, 50), fontsize = 20, color = 'white')
# plt.zlabel('real part', fontsize = 20, fontweight = 'bold')
plt.xlim(-3*_kappa,3.*_kappa)
plt.ylim(-3*_kappa,3.*_kappa)
#plt.colorbar()
cbar = plt.colorbar()
#cbar.set_label(label = r'$\rho_{\uparrow}$', size = 38)
ticks = [0, 10, 20, 30]
cbar.set_ticks(ticks)
cbar.set_ticklabels(ticks)
cbar.ax.tick_params(labelsize=20)
#plt.savefig('Fig2a_86mm_n_k.eps', dpi = 320)
plt.savefig('Fig3b.eps', dpi = 350)
# plt.legend()
...
#plt.figure(4)
plt.figure(figsize=(3.38583, 3.38583))
plt.errorbar(kr_uniq, n_kr.real, n_kr_errs.real, marker='o', markersize = 6, elinewidth=2.00, linewidth = 0.00, color = 'black', label=r'$\langle N(k_{r}) \rangle$')
#plt.title('Polar Averaged Momentum Distribution, ' + r'$\tilde T = ' + str(np.round(T_,2)) + '$', fontsize = 22)
plt.xlabel('$k_{r}$', fontsize = 24, fontweight = 'bold')
...
plt.axvline(x = _kappa, color = 'r', linewidth = 1.5, linestyle='dashed', label = r'$k_{r} = \tilde{\kappa} $')
#plt.savefig('n_k_plane_wave_1D.eps')
plt.xlim(0.,3.*_kappa)
plt.legend()
