...
if d == 2:
    ...
    plt.errorbar(list_x[0:stop_indx]/lam_T, corr_sorted[0:stop_indx].real/corr_sorted[0].real, normalized_errs[0:stop_indx], marker='o', markersize = 6, elinewidth=0.5, linewidth=0.5, color = 'r', label = 'CL')

# Fit the data to a powerlaw 
pars,cov = curve_fit(f=exponential_fit, xdata=list_x[0:stop_indx]/lam_T, ydata=corr_sorted[0][0:stop_indx].real/corr_sorted[0][0].real, p0=[0,0], bounds=(-np.inf, np.inf))
#pars,cov = curve_fit(f=exponential_fit, xdata=list_x[0:stop_indx], ydata=corr_sorted[0][0:stop_indx].real/corr_sorted[0][0].real, p0=[0,0], bounds=(-np.inf, np.inf))
print(pars[0])
print(pars[1])
lam_T = 8.7262735727e-01
#plt.plot(list_x[0:stop_indx]/lam_T, pars[0] * (list_x[0:stop_indx]/lam_T)**(pars[1]), '-k', linewidth = 0.5, label = 'Exponential Fit: $ e^{' + str(round(pars[1],2)) + '}$')
plt.plot(list_x[0:stop_indx]/lam_T, pars[0] * exponential_fit(list_x[0:stop_indx]/lam_T, pars[0], pars[1]), color = 'r', linestyle='--', linewidth = 1.0, label = 'Exponential fit: ' + r'$\frac{\xi}{\lambda_{T}} = ' + str(round(-1. * pars[1]/lam_T, 2)) + '$') 
plt.axhline(y = cond_frac, ls='--',color = 'k', linewidth = 0.75, label = '$N_{k = 0}/N$')
...
plt.title('Normalized Correlation Function, $T > T_{c}$', fontsize = 16)
plt.xlabel('$r$', fontsize = 20, fontweight = 'bold') # actually "x"
...
plt.ylim(-0.05, 1.05)
plt.legend()
#plt.savefig('T_high_2D_corr_fxn.eps')
plt.show()
