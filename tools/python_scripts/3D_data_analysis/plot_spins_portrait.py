...
# Average the data
    ...
    Sy_vector, Sy_errs = calculate_field_average(S_y_real + 1j*S_y_imag, N_spatial, Nsamples_to_avg, start_index)
...
# Redefine numpy array post sorting
    Sx = np.array(d_frame_S['Sx']) # should be a flattened function of space  
    Sy = np.array(d_frame_S['Sy']) # should be a flattened function of space  

    Sz = np.array(d_frame_S['Sz'])
    #Sz_sorted.resize(Nx, Nx)
    #Sz_sorted = np.transpose(Sz_sorted)

    spin_profiles = [Sx, Sy, Sz]
    suffix = 'T_' + str(np.round(T_,2)) + '_start_' + str(start_index) + '_avglen' + str(Nsamples_to_avg) + '_'
    #fname_str = ['rho_0_', 'rho_1_']
    fname_str = ['Sx_', 'Sy_', 'Sz_']
    # Now save to VTK 
    for i, profile in enumerate(spin_profiles):
      # Do real part  
