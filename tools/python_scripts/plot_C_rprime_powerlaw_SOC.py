...
def power_law_cos(x, a, b, C):
    return a * np.cos(C * x) / np.power(x, b) 


def exp_law_cos(x, a, b, C):
    return a * np.cos(C * x) * np.exp(-b * x) / x
...
correlation_data = 'C_rprime_0.dat' # This is a function of "r'", the coordinate distance away from a reference site 'r' 
#correlation_data = 'Sx_r_Sx_0_alt.dat' # This is a function of "r'", the coordinate distance away from a reference site 'r' 
#correlation_data = 'C_rprime_01.dat' # This is a function of "r'", the coordinate distance away from a reference site 'r' 
#correlation_data = 'C_rprime_tot.dat' # This is a function of "r'", the coordinate distance away from a reference site 'r' 
...
if d == 2:
    ...
    plt.errorbar(list_x[0:stop_indx]/lam_T, corr_sorted[0][0:stop_indx].real/corr_sorted[0][0].real, errs_sorted[0][0:stop_indx]/corr_sorted[0][0].real, marker = 'o', markersize = 6, elinewidth=0.5, linewidth = 0.0,  color = 'r', label = 'Langevin Numerics')
elif d == 1:
...
do_power_law_cos = True
if(do_power_law_cos):
  pars,cov = curve_fit(f=power_law_cos, xdata=list_x[1:stop_indx]/lam_T, ydata=corr_sorted[0][1:stop_indx].real/corr_sorted[0][0].real, p0=[1.,1.0,2.0], bounds=(-np.inf, np.inf))
else:
  pars,cov = curve_fit(f=exp_law_cos, xdata=list_x[1:stop_indx]/lam_T, ydata=corr_sorted[0][1:stop_indx].real/corr_sorted[0][0].real, p0=[1.,0.5,1.2], bounds=(-np.inf, np.inf))
print(pars[0])
print(pars[1])
print(pars[2])
#plt.plot(list_x[1:stop_indx]/lam_T, pars[0] * np.cos( pars[2] * list_x[1:stop_indx]/lam_T) / np.power(list_x[1:stop_indx]/lam_T, pars[1]), color='r', ls = '--', linewidth = 2.0, label = r'Fit: $A$ cos$(B r) / r^{\eta}$')

if(do_power_law_cos):
  #plt.plot(list_x[1:stop_indx]/lam_T, pars[0] * np.cos( 1.0 * list_x[1:stop_indx]/lam_T) / np.power(list_x[1:stop_indx]/lam_T, 1.), color='r', ls = '--', linewidth = 2.0, label = r'Fit: $A$ cos$(B r) / r^{\eta}$')
  #plt.plot(list_x[1:stop_indx]/lam_T, pars[0] * np.cos( pars[2] * list_x[1:stop_indx]/lam_T) / np.power(list_x[1:stop_indx]/lam_T, pars[1]), color='r', ls = '--', linewidth = 2.0, label = r'Fit: $A$ cos$(B r) / r^{\eta}$')
  plt.plot(list_x[1:stop_indx]/lam_T, pars[0] * np.cos( pars[2] * list_x[1:stop_indx]/lam_T) / np.power(list_x[1:stop_indx]/lam_T, pars[1]), color='r', ls = '--', linewidth = 2.0, label = r'Fit: $A$ cos$(B r) / r^{\eta}$')
else:
  plt.plot(list_x[1:stop_indx]/lam_T, exp_law_cos(list_x[1:stop_indx]/lam_T, pars[0], pars[1], pars[2]), color='r', ls = '--', linewidth = 2.0, label = r'Fit: $\frac{A cos (B r) e^{-r/l}}{r}$')

#plt.annotate('$A = ' + str(np.round(pars[0],2)) + '$, $\eta = ' + str(-np.round(pars[1],2)) + '$, $B = ' + str(np.round(pars[2], 2)) + '$', (0.5,0.5))
if(do_power_law_cos):
  #plt.annotate('$A = ' + str(np.round(pars[0],2)) + '$, $\eta = ' + str(np.round(0.87,2)) + '$, $B = ' + str(np.round(1.85, 2)) + '$', (0.5,0.5))
  plt.annotate('$A = ' + str(np.round(pars[0],2)) + '$, $\eta = ' + str(np.round(pars[1],2)) + '$, $B = ' + str(np.round(pars[2], 2)) + '$', (0.5,0.5))
else:
  plt.annotate('$A = ' + str(np.round(pars[0],2)) + '$, $l = ' + str(np.round(1./pars[1],2)) + '$, $B = ' + str(np.round(pars[2], 2)) + '$', (1.0,0.2))
...
plt.title('Corr. Function, ' + r'$\tilde T = ' + str(T_) + '$', fontsize = 24)
plt.xlabel(r'$r = |r - r^{\prime}|$', fontsize = 26, fontweight = 'bold') # actually "x"
plt.ylabel(r'$C(r)/C(0)$', fontsize = 26, fontweight = 'bold')
# plt.zlabel('real part', fontsize = 20, fontweight = 'bold')
#plt.colorbar()
#plt.xlim(-1, 1)
plt.legend()
