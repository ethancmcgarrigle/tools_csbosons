...
up_density = up_real + 1j * up_imag

N_samples = int(len(up_density)/(Nx*Ny))


# Factor the files into N samples
# Use split (np) to get arrays that represent each sample (1 array per sample) Throw out the first sample (not warmed up properly) 
sample_arrays = np.split(up_density, N_samples) 
...
gnuplot_script = 'print_up_density_png.p'
gnuplot_script_cp = 'print_up_density_png_cp.p'
gnuplot_command = 'gnuplot ' + gnuplot_script_cp

dir_ = 'up_density_images'
