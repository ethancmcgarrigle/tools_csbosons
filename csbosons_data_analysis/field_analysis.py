...
def calculate_field_average(field_data: np.ndarray, N_spatial: int, N_samples_to_avg: int) -> tuple: 
    ...
    if(N_samples == 1):
      ...
      print('Averaging: ' + str(N_samples_to_avg) + ' samples')
...
def extract_grid(field_data: np.ndarray) -> tuple:
...
def process_data(file_list: list, N_spatial: int, d: int, CL: bool, N_samples_to_avg = 5: int) -> tuple:
    ...
    for i, file in enumerate(file_list): 
      file_data[i] = np.loadtxt(file, unpack=True)

    # Extract the spatial (r or k) grid  
    grid = extract_grid(file_data[0]) 
    ...
    if(_CL and N_samples_to_avg > N_samples):
