...
def exp_law_cos(x, a, b, C):
    return a * np.cos(C * x) * np.exp(-b * x)
...
if d == 2:
    ...
    plt.errorbar(list_x[0:stop_indx]/lam_T, corr_sorted[0][0:stop_indx].real/corr_sorted[0][0].real, errs_sorted[0][0:stop_indx]/corr_sorted[0][0].real, marker = 'o', markersize = 6, elinewidth=0.5, linewidth = 0.0,  color = 'r', label = 'CL')
elif d == 1:
...
if(do_power_law_cos):
  pars,cov = curve_fit(f=power_law_cos, xdata=list_x[1:stop_indx]/lam_T, ydata=corr_sorted[0][1:stop_indx].real/corr_sorted[0][0].real, p0=[1.,0.5,1.2], bounds=(-np.inf, np.inf))
else:
...
if(do_power_law_cos):
  ...
  plt.plot(list_x[1:stop_indx]/lam_T, exp_law_cos(list_x[1:stop_indx]/lam_T, pars[0], pars[1], pars[2]), color='r', ls = '--', linewidth = 2.0, label = r'Fit: $A$ cos$(B r) e^{-C r}$')

plt.annotate('$A = ' + str(np.round(pars[0],2)) + '$, $\eta = ' + str(-np.round(pars[1],2)) + '$, $B = ' + str(np.round(pars[2], 2)) + '$', (15.0,0.5))
...
plt.xlabel(r'$r$', fontsize = 20, fontweight = 'bold') # actually "x"
plt.ylabel(r'$C(r)/C(0)$', fontsize = 20, fontweight = 'bold')
# plt.zlabel('real part', fontsize = 20, fontweight = 'bold')
#plt.colorbar()
#plt.xlim(-1, 1)
plt.legend()
