...
def calculate_field_average(field_data, Nx, Ny, dim, N_samples_to_avg, apply_ADT=False, scale_factor=np.ones(100), weights = np.ones(100), summed_weights = 1000): # assumes cubic/square mesh 
    ...
    if(not apply_ADT):
      scale_factor = np.ones(len(sample_arrays))
     
    if(len(scale_factor) != len(sample_arrays)):
      scale_factor = np.ones(len(sample_arrays))
    #assert(len(scale_factor) == len(sample_arrays))
    
    # Scale all of the observables after column 3
    # TODO remove forloop implementation
    N_field_elements = Nx*Ny    # Previous assumes CUBIC/SQUARE mesh 
...
def doStats(filename, N_input_to_throw, ops_file, input_file, scalar_avgs_file):
 ...
 #    
    ...
    N_samples = int(len(nk_real)/(Nx*Ny))
 ...
 #      
    ...
    if(apply_ADT):
        avgd_data, errs = calculate_field_average(nk_data, Nx, Ny, d, N_samples - N_input_to_throw - 1, apply_ADT, scale_factor, t_elapsed[N_input_to_throw+1:len(t_elapsed)], total_t_elapsed)
    else:
        avgd_data, errs = calculate_field_average(nk_data, Nx, Ny, d, int(0.80*N_samples)) 
    
    Nk_max = max(avgd_data)
    max_indx = np.where(avgd_data == Nk_max)[0][0]
    assert(avgd_data[max_indx] == Nk_max) 
    Nk_max_err = errs[max_indx]
    # Using the averaged data, find the two largest peaks. If plane wave, the second largest peak will be really small. If stripe, both will be macroscopic
    # This will tell you whether you have a stripe or plane wave, but not a mu phase (ring) 
    
    data = {'kx': kx, 'ky': ky, 'n_k': avgd_data, 'n_k_errs': errs}
    ...
    kx_vec = (d_frame['kx'].iloc[indices]) 
    n_kx = (d_frame['n_k'].iloc[indices])
    n_kx_errs = (d_frame['n_k_errs'].iloc[indices])

    # Find the two largest peaks 
    n_max = np.partition(n_kx, -2)[-1]
    n_pmax = np.partition(n_kx, -2)[-2] # penultimate max 

    #print('2 maxes')
    #print(n_max)
    #print(n_pmax)

    # find the indices where these occur 
    kx_indx_nmax = np.where(n_kx == n_max)[0][0]
    kx_indx_pnmax = np.where(n_kx == n_pmax)[0][0]

    #print(kx_vec.iloc[kx_indx_nmax]) # correct syntax 

    if(kx_vec.iloc[kx_indx_nmax] == 0.):   
      # we've grabbed a k = 0 peak, we dont want this. discard, get the next largests  
      n_max = np.partition(n_kx, -2)[-2] 
      n_pmax = np.partition(n_kx, -3)[-3] 

    #print('2 maxes')
    #print(n_max)
    #print(n_pmax)
    # find the indices where these occur, redo  
    kx_indx_nmax = np.where(n_kx == n_max)[0][0]
    kx_indx_pnmax = np.where(n_kx == n_pmax)[0][0]
    ...
    noise_avgd_Nkappa += n_max 
    noise_avgd_Nkappa += n_pmax 

    err_NKappa = 0. + 1j* 0.
    #print(kx_indx_nmax)
    #print(n_kx_errs)
    err_NKappa += n_kx_errs.iloc[kx_indx_nmax]
    err_NKappa += n_kx_errs.iloc[kx_indx_pnmax]
 ...
 #        filehandle.write("{} {} {} {}\n".format(noise_avgd_Nkappa.real, err_NKappa.real, SOC_frac, SOC_frac_err))
    sys.stdout.write("{0} {1} {2} {3} {4} {5}\n".format(noise_avgd_Nkappa.real, err_NKappa.real, SOC_frac, SOC_frac_err, Nk_max.real, Nk_max_err.real))
    #sys.stdout.write("\n")
